...
- Modify only if an architecture invariant changes before verification freeze: `docs/superpowers/specs/2026-07-16-ktm-xc2-integration-design.md`
- Modify if either OFF job lacks the required source-list proof or bounded build reliability: `.github/workflows/ktm-xc2-foundation.yml`
- Modify ignored ledgers/checkers/raw evidence under `.superpowers/sdd/` as required.

Task 6 follows the Phase 1 two-SHA evidence model. `verificationSha` is the immutable implementation/workflow object that passes clean local verification and whole-slice review before it is pushed. The exact three-job CI run binds only to `verificationSha`. A later plan-only `evidenceSha` records that immutable run; it must not claim that the cited CI run tested the evidence commit or current evidence HEAD.

The predecessor candidate `f7e4d3324d05cd0616eed7f1586db3f93f8f3ce0` proved the OFF cache/target/`rx14` source-list contract, but its exact push run `29684921049` failed twice in `Linux / KTM OFF` at 98% under unbounded `cmake --build build --parallel`. Both attempts ended with a runner shutdown signal and exit `143`, without a compiler or linker diagnostic, and skipped CTest. Bound Linux to three build workers before forming the next candidate. This is an operational CI reliability correction, not an architecture-invariant change, so the integration design remains unchanged. The failed predecessor run is provenance only and cannot satisfy Step 8 for the next candidate.

- [ ] **Step 1: Close the OFF workflow contract before final verification.** Ensure both `Linux / KTM OFF` and `macOS / KTM OFF` query file-api cache/codemodel data, require `RX14_KTM_XC2:BOOL=OFF`, reject Qt Test/WebSockets discovery and KTM/XC2 target names, open the `rx14` target codemodel, and reject every normalized `src/ktm` source path. Require each job to print a decisive `PASS: rx14 codemodel contains no src/ktm sources` line and build with exactly `cmake --build build --parallel 3`. If the workflow changes, update the ignored semantic/mutation checks so deleting either platform's source-list assertion or restoring unbounded parallelism on either OFF job fails, then run the canonical checker, mutations, and pinned actionlint before forming a verification candidate.
- [ ] **Step 2: Establish a clean, space-safe verification candidate.** Require a clean worktree and at least 1.00 GiB free on `G:` before each configure. Use only fresh, initially absent `build-vci-final-on` and `build-vci-final-off` directories, run them sequentially with Release and local `--parallel 3`, and keep raw logs under ignored `.superpowers/sdd/vci-task-6-evidence`. Never reuse Task 5 or Phase 1 build trees. Record the full candidate SHA in every local evidence artifact; any tracked change invalidates that candidate and its prior logs.
- [ ] **Step 3: Run the complete clean Windows ON verification.** Configure `build-vci-final-on` with `RX14_KTM_XC2=ON` and `BUILD_TESTING=ON`, build the complete graph, and run unfiltered CTest. Record exact configure/build/test exit codes, test count, failures, duration, compiler, Qt, CMake, generator, UTC timestamp, and candidate SHA. This local unfiltered result is distinct from the later Windows CI command `ctest -L ktm`; neither starts or validates a real XC2/JAR/VCI/ECU session.
- [ ] **Step 4: Run the complete clean Windows OFF proof.** Configure `build-vci-final-off` with `RX14_KTM_XC2=OFF`, issue file-api cache/codemodel queries, prove no Qt Test/WebSockets discovery, no KTM/XC2 target, exactly one `rx14` target inspection, and no normalized `src/ktm` source in its source list, then build the complete OFF graph. If unfiltered OFF CTest is also run, record it as a separate base-regression result rather than conflating it with the required OFF isolation proof or the CI suites.
- [ ] **Step 5: Perform whole-slice hygiene and provenance checks.** Against the exact range `82255dc..candidate`, run `git diff --check`, enumerate changed paths and binary deltas, inspect sensitive-looking tracked paths/content locations without copying secret values, verify synthetic fixtures remain `liveCapture:false`, and adjudicate every match. Any vendor binary, firmware, credential, secret, or misleading live-capture claim blocks verification freeze.
- [ ] **Step 6: Complete whole-slice review before freezing or pushing.** Review `82255dc..candidate` for frozen REST/STOMP contract fidelity, same-authority/Cookie isolation, request/session/generation/selection-epoch safety, UI lifecycle, ON/OFF graph isolation, test provenance, and evidence accuracy. Fix every Critical or Important finding with a failing regression test where behavior changes, then treat the changed HEAD as a new candidate: rerun every affected complete local ON/OFF/workflow verification, repeat hygiene, and re-review. Do not designate `verificationSha` while any Critical/Important finding or candidate-changing edit remains.
- [ ] **Step 7: Freeze and push the immutable verification SHA.** Only after Steps 1-6 pass, designate the clean full 40-character HEAD as `verificationSha`, confirm all retained local evidence names that exact object, and push `${verificationSha}:refs/heads/feature/ktm-xc2-integration` to `fork`. Verify the remote branch resolves to the same object. Any later implementation, workflow, design-invariant, or review-fix change abandons this designation and returns to the candidate/local-verification/review cycle.
- [ ] **Step 8: Require the exact verification-SHA three-job run.** Discover only the push run whose event is `push` and whose `headSha` exactly equals `verificationSha`. Require exactly `Windows / KTM ON`, `Linux / KTM OFF`, and `macOS / KTM OFF`, all completed/success, with immutable run/job URLs. Treat Windows CI's labeled `ctest -L ktm` as separate from Step 3's local unfiltered CTest. Require both OFF jobs to show the cache/target and `rx14` source-list proofs and to keep `cmake --build build --parallel 3`; macOS also keeps its 45-minute timeout. Never substitute a nearby SHA, PR run, evidence HEAD, or differently named job set.
- [ ] **Step 9: Preserve the eight HIL unknowns as open evidence items.** Record without claiming resolution: actual per-route 200/204/body behavior; device DTO nullability/stability; actual hardware enumeration/provider string; progress ordering/replay/loss; raw voltage unit/update rate; apply-to-selected/connected/valid-voltage timing; close-to-disconnected/selected-clear timing; and real HTTP/D-PDU/VCI-removal/device-disconnect error behavior. State that no real XC2/JAR/AVL DiTEST VCI2K/ECU hardware run occurred and that fake/golden tests validate only the frozen software contract and ordering defenses.
- [ ] **Step 10: Record the run in a separate evidence SHA without recursion.** After Step 8 succeeds, write transient counts, durations, review disposition, `verificationSha`, run/job IDs and URLs, decisive job facts, and open-HIL wording in this plan; update only the ignored SDD ledger/raw evidence alongside it. Runtime evidence belongs in the plan. Change the design only when an architecture invariant changes, and do so before repeating verification freeze rather than in this evidence-only step. Stage the plan only and commit with `rtk git commit -m "docs: record XC2 VCI connection verification"`; that resulting commit is `evidenceSha`. The cited run remains evidence for `verificationSha` only. Do not claim it covers `evidenceSha`, and do not recursively create another evidence commit merely to record a run triggered by pushing `evidenceSha`.

Only after the exact run facts and open-HIL statement are present may Task 6 and the applicable Completion Gate boxes be checked in `evidenceSha`.
...
- [ ] The clean `verificationSha` passes the local Windows ON full build plus unfiltered CTest and the fresh OFF cache/target/`rx14`-source codemodel proof plus full build.
- [ ] The exact `verificationSha` has the required successful Windows/Linux/macOS CI jobs; both OFF jobs prove the `rx14` source list contains no `src/ktm`, and the Windows CI `-L ktm` result is recorded separately from local unfiltered CTest.
- [ ] A later plan-only `evidenceSha` records the immutable `verificationSha` run without claiming that run tested the evidence commit or evidence HEAD.
